...
extern void Configurar_SSI2(void)
{
    ...
    GPIOD->AFSEL = (1<<7)|(1<<6)|(1<<5)|(1<<4); // porque se usa el pin B4-B7
    GPIOD->PCTL = (GPIOD->PCTL&0x0000FFFF) | 0xFFFF0000; // tabla p.689 *se apaga del pin 0-3 y se enciende del pin 4-7
    //GPIOD_AHB->PCTL &= 0xFFFF0000;
    //GPIOD_AHB->PCTL |= 0x0000FFFF; // tabla p.688
    GPIOD->DEN |= (1<<7)|(1<<6)|(1<<5)|(1<<4);
    //                MISO    MOSI    CS    SCLK
